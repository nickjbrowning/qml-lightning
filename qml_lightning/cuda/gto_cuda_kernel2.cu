...
__global__
void egto_atomic_representation_cuda(const torch::PackedTensorAccessor32<float, 3, torch::RestrictPtrTraits> coordinates,
		const torch::PackedTensorAccessor32<float, 2, torch::RestrictPtrTraits> charges,
		const torch::PackedTensorAccessor32<float, 1, torch::RestrictPtrTraits> species,
		const torch::PackedTensorAccessor32<int, 2, torch::RestrictPtrTraits> element_types,
		const torch::PackedTensorAccessor32<int, 1, torch::RestrictPtrTraits> blockAtomIDs, // blockIdx -> atom idx
...
//		}
		...
		for (int k = threadIdx.x; k < norbs * ngauss; k += blockDim.x) {
			...
			float val = sqrt_eta * (1.0 / powf(rij, inv_factors[gto_power] + gto_power)) * ang * cut;
			...
			for (int m = 0; m < nspecies; m++) {
				...
				int kidx = korb * nmbody * ngauss + mnidx * ngauss + z;
	...
	for (int jatom = 0; jatom < nneighbours_i; jatom++) {
		...
		for (int x = 0; x < 3; x++) {
			...
			for (int k = threadIdx.x; k < norbs * ngauss; k += blockDim.x) {
				...
				float rscaling = (1.0 / powf(rij, inv_factors[gto_power] + gto_power));
				float drscaling = -(inv_factors[gto_power] + float(gto_power)) * (1.0 / powf(rij, 1.0 + inv_factors[gto_power] + float(gto_power)));
				...
				if (x == 0) {
					if (gto_component_x == 1.0) {
						dang[0] = -1.0 * powf(drij[1], gto_component_y) * powf(drij[2], gto_component_z);
					} else if (gto_component_x > 1.0) {
						dang[0] = -gto_component_x * powf(drij[0], gto_component_x - 1.0) * powf(drij[1], gto_component_y) * powf(drij[2], gto_component_z);
					}
				} else if (x == 1) {
					if (gto_component_y == 1.0) {
						dang[1] = -1.0 * powf(drij[0], gto_component_x) * powf(drij[2], gto_component_z);
					} else if (gto_component_y > 1.0) {
						dang[1] = -gto_component_y * powf(drij[1], gto_component_y - 1.0) * powf(drij[0], gto_component_x) * powf(drij[2], gto_component_z);
					}
				} else {
					if (gto_component_z == 1.0) {
						dang[2] = -1.0 * powf(drij[0], gto_component_x) * powf(drij[1], gto_component_y);
					} else if (gto_component_z > 1.0) {
						dang[2] = -gto_component_z * powf(drij[2], gto_component_z - 1.0) * powf(drij[0], gto_component_x) * powf(drij[1], gto_component_y);
					}
				}
				...
				float radial = exp(-eta * powf(rij - sgridpoints[z], 2.0));
				float dradial = -2.0 * eta * (rij - sgridpoints[z]);

				float val = sqrt_eta * (1.0 / powf(rij, 2.0 + gto_power)) * ang * cut;
...
void EGTOCuda_old(torch::Tensor coordinates, torch::Tensor charges, torch::Tensor species, torch::Tensor element_types, torch::Tensor blockAtomIDs,
	...
	int shared_mem_size = nspecies * nspecies + ngaussians + 5 * norbs + (lmax + 1) * nmbody * ngaussians + nmbody * ngaussians * norbs + 18;
...
void EGTODerivativeCuda_old(torch::Tensor coordinates, torch::Tensor charges, torch::Tensor species, torch::Tensor element_types, torch::Tensor blockAtomIDs,
	...
	int shared_mem_size = nspecies * nspecies + ngaussians + 5 * norbs + (2 * (lmax + 1) * nmbody * ngaussians) + 2 * (nmbody * ngaussians * norbs) + 18;
