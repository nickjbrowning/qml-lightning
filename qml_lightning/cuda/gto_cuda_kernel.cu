...
__global__
void egto_atomic_representation_derivative_cuda_ver2(const torch::PackedTensorAccessor32<float, 3, torch::RestrictPtrTraits> coordinates,
		const torch::PackedTensorAccessor32<float, 2, torch::RestrictPtrTraits> charges,
		const torch::PackedTensorAccessor32<float, 1, torch::RestrictPtrTraits> species,
		const torch::PackedTensorAccessor32<int, 2, torch::RestrictPtrTraits> element_types,
		const torch::PackedTensorAccessor32<int, 1, torch::RestrictPtrTraits> blockAtomIDs, // blockIdx -> atom idx
		...
		const torch::PackedTensorAccessor32<float, 1, torch::RestrictPtrTraits> gridpoints,
		const torch::PackedTensorAccessor32<float, 1, torch::RestrictPtrTraits> lchannel_weights,
		const torch::PackedTensorAccessor32<float, 1, torch::RestrictPtrTraits> inv_factors,
		const torch::PackedTensorAccessor32<float, 1, torch::RestrictPtrTraits> etas, int lmax, float rcut, float rswitch,
		const torch::PackedTensorAccessor32<float, 3, torch::RestrictPtrTraits> cell,
		const torch::PackedTensorAccessor32<float, 3, torch::RestrictPtrTraits> inv_cell, int cutoff_type, int distribution_type,
		torch::PackedTensorAccessor32<float, 3, torch::RestrictPtrTraits> output, torch::PackedTensorAccessor32<float, 5, torch::RestrictPtrTraits> grad) {
...
//need to generate the representation partially first.
	...
	for (int k = threadIdx.x; k < norbs * ngauss; k += blockDim.x) {
		...
		float inv_factor = inv_factors[z];
		float eta = etas[z];
		float gto_component_x = sgto_components_x[korb];
		float gto_component_deriv_x = (gto_component_x == 1.0) ? -1.0 : -gto_component_x;
...
__global__ void egto_atomic_representation_cuda_ver2(const torch::PackedTensorAccessor32<float, 3, torch::RestrictPtrTraits> coordinates,
		const torch::PackedTensorAccessor32<float, 2, torch::RestrictPtrTraits> charges,
		const torch::PackedTensorAccessor32<float, 1, torch::RestrictPtrTraits> species,
		const torch::PackedTensorAccessor32<int, 2, torch::RestrictPtrTraits> element_types,
		const torch::PackedTensorAccessor32<int, 1, torch::RestrictPtrTraits> blockAtomIDs, // blockIdx -> atom idx
...
__global__ void egto_atomic_representation_cuda_ver3(const torch::PackedTensorAccessor32<float, 3, torch::RestrictPtrTraits> coordinates,
		const torch::PackedTensorAccessor32<float, 2, torch::RestrictPtrTraits> charges,
		const torch::PackedTensorAccessor32<float, 1, torch::RestrictPtrTraits> species,
		const torch::PackedTensorAccessor32<int, 2, torch::RestrictPtrTraits> element_types,
		const torch::PackedTensorAccessor32<int, 1, torch::RestrictPtrTraits> blockAtomIDs, // blockIdx -> atom idx
		...
		const torch::PackedTensorAccessor32<int, 1, torch::RestrictPtrTraits> gto_powers,
		const torch::PackedTensorAccessor32<float, 1, torch::RestrictPtrTraits> gridpoints,
		const torch::PackedTensorAccessor32<float, 1, torch::RestrictPtrTraits> lchannel_weights, float inv_factor, float eta, int lmax, float rcut,
		float rswitch, const torch::PackedTensorAccessor32<float, 3, torch::RestrictPtrTraits> cell,
		const torch::PackedTensorAccessor32<float, 3, torch::RestrictPtrTraits> inv_cell, int cutoff_type, int distribution_type,
		torch::PackedTensorAccessor32<float, 3, torch::RestrictPtrTraits> output) {
...
//load coordinates into shared memory
	...
	for (int k = threadIdx.x; k < norbs * ngauss; k += blockDim.x) {
		...
		float weighted_inv_factor = powf(inv_factor, lchannel + 1.0);
		float weighted_eta = powf(eta, lchannel + 1.0);
		...
		for (int jatom = 0; jatom < nneighbours_i; jatom++) {
			...
			float val = (1.0 / powf(rij, weighted_inv_factor + gto_power)) * ang * cut;

			float sqrt_eta = sqrt(weighted_eta / M_PI);

			float g = get_radial_distribution(rij, weighted_eta, sgridpoints, z, distribution_type);
...
__global__
void egto_atomic_representation_derivative_cuda_ver3(const torch::PackedTensorAccessor32<float, 3, torch::RestrictPtrTraits> coordinates,
		const torch::PackedTensorAccessor32<float, 2, torch::RestrictPtrTraits> charges,
		const torch::PackedTensorAccessor32<float, 1, torch::RestrictPtrTraits> species,
		const torch::PackedTensorAccessor32<int, 2, torch::RestrictPtrTraits> element_types,
		const torch::PackedTensorAccessor32<int, 1, torch::RestrictPtrTraits> blockAtomIDs, // blockIdx -> atom idx
...
//need to generate the representation partially first.
	...
	for (int k = threadIdx.x; k < norbs * ngauss; k += blockDim.x) {
		...
		for (int jatom = 0; jatom < nneighbours_i; jatom++) {
			...
			float rscaling = (1.0 / powf(rij, weighted_inv_factor + gto_power));
			float drscaling = -(weighted_inv_factor + float(gto_power)) * (1.0 / powf(rij, 1.0 + weighted_inv_factor + float(gto_power)));
			float ang = powf(drij[0], gto_component_x) * powf(drij[1], gto_component_y) * powf(drij[2], gto_component_z);
			...
			float radial = get_radial_distribution(rij, weighted_eta, sgridpoints, z, distribution_type);
			...
			for (int x = 0; x < 3; x++) {
				...
				float dradial = get_radial_derivative_distribution(drijx[x], rij, weighted_eta, sgridpoints, z, distribution_type);
...
__global__ void egto_atomic_representation_cuda_ver4(const torch::PackedTensorAccessor32<float, 3, torch::RestrictPtrTraits> coordinates,
		const torch::PackedTensorAccessor32<float, 2, torch::RestrictPtrTraits> charges,
		const torch::PackedTensorAccessor32<float, 1, torch::RestrictPtrTraits> species,
		const torch::PackedTensorAccessor32<int, 2, torch::RestrictPtrTraits> element_types,
		const torch::PackedTensorAccessor32<float, 2, torch::RestrictPtrTraits> element_vectors,
		const torch::PackedTensorAccessor32<int, 1, torch::RestrictPtrTraits> blockAtomIDs, // blockIdx -> atom idx
		const torch::PackedTensorAccessor32<int, 1, torch::RestrictPtrTraits> blockMolIDs, // blockIdx -> molecule jdx
		const torch::PackedTensorAccessor32<int, 3, torch::RestrictPtrTraits> neighbourlist,
		const torch::PackedTensorAccessor32<int, 2, torch::RestrictPtrTraits> nneighbours, const int max_neighbours,
	...
	int molID = blockMolIDs[blockIdx.x];
	int iatom = blockAtomIDs[blockIdx.x];
	int iatom_type = element_types[molID][iatom];
	int nneighbours_i = nneighbours[molID][iatom];
...
//load coordinates into shared memory
	...
	for (int k = threadIdx.x; k < norbs * ngauss; k += blockDim.x) {
		...
		for (int jatom = 0; jatom < nneighbours_i; jatom++) {
			...
			for (int h = 0; h < element_vectors.size(1); h++) {
				coeff += element_vectors[iatom_type][h] * element_vectors[element_type][h];
			}
			...
			for (int m = 0; m < nspecies; m++) {
				...
				local_rep[mnidx * blockDim.x + threadIdx.x] += coeff * gval;
...
__global__
void egto_atomic_representation_derivative_cuda_ver4(const torch::PackedTensorAccessor32<float, 3, torch::RestrictPtrTraits> coordinates,
		const torch::PackedTensorAccessor32<float, 2, torch::RestrictPtrTraits> charges,
		const torch::PackedTensorAccessor32<float, 1, torch::RestrictPtrTraits> species,
		const torch::PackedTensorAccessor32<int, 2, torch::RestrictPtrTraits> element_types,
		const torch::PackedTensorAccessor32<float, 2, torch::RestrictPtrTraits> element_vectors,
...
void EGTOCuda_ver2(torch::Tensor coordinates, torch::Tensor charges, torch::Tensor species, torch::Tensor element_types, torch::Tensor blockAtomIDs,
		torch::Tensor blockMolIDs, torch::Tensor neighbourlist, torch::Tensor nneighbours, torch::Tensor mbodylist, torch::Tensor gto_components,
		torch::Tensor gto_powers, torch::Tensor orbital_weights, torch::Tensor gridpoints, torch::Tensor lchannel_weights, torch::Tensor inv_factors,
...
//printf("Shared mem requested: %d bytes\n", shared_mem_size);
	egto_atomic_representation_cuda_ver2<<<currBatch, nthreads, shared_mem_size * sizeof(float)>>>(
			coordinates.packed_accessor32<float, 3, torch::RestrictPtrTraits>(),
			charges.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
			species.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
			element_types.packed_accessor32<int, 2, torch::RestrictPtrTraits>(),
			...
			gridpoints.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
			lchannel_weights.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
			inv_factors.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
			etas.packed_accessor32<float, 1, torch::RestrictPtrTraits>(), lmax,
			rcut, rswitch,
			cell.packed_accessor32<float, 3, torch::RestrictPtrTraits>(),
			inv_cell.packed_accessor32<float, 3, torch::RestrictPtrTraits>(), cutoff_type, distribution_type,
			gto_output.packed_accessor32<float, 3, torch::RestrictPtrTraits>());
...
//printf("CURR BATCH: %i, NTHREADS: %i\n", currBatch, nthreads);
	...
	egto_atomic_representation_derivative_cuda_ver2<<<currBatch, nthreads, shared_mem_size * sizeof(float)>>>(
			coordinates.packed_accessor32<float, 3, torch::RestrictPtrTraits>(),
			charges.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
			species.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
			element_types.packed_accessor32<int, 2, torch::RestrictPtrTraits>(),
			...
			gridpoints.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
			lchannel_weights.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
			inv_factors.packed_accessor32<float, 1, torch::RestrictPtrTraits>(), etas.packed_accessor32<float, 1, torch::RestrictPtrTraits>(), lmax,
			rcut, rswitch, cell.packed_accessor32<float, 3, torch::RestrictPtrTraits>(),
			inv_cell.packed_accessor32<float, 3, torch::RestrictPtrTraits>(), cutoff_type, distribution_type,
			gto_output.packed_accessor32<float, 3, torch::RestrictPtrTraits>(), gto_output_derivative.packed_accessor32<float, 5, torch::RestrictPtrTraits>());
...
//printf("Shared mem requested: %d bytes\n", shared_mem_size);
	egto_atomic_representation_cuda_ver3<<<currBatch, nthreads, shared_mem_size * sizeof(float)>>>(
			coordinates.packed_accessor32<float, 3, torch::RestrictPtrTraits>(),
			charges.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
			species.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
			element_types.packed_accessor32<int, 2, torch::RestrictPtrTraits>(),
...
//printf("CURR BATCH: %i, NTHREADS: %i\n", currBatch, nthreads);
	...
	egto_atomic_representation_derivative_cuda_ver3<<<currBatch, nthreads, shared_mem_size * sizeof(float)>>>(
			coordinates.packed_accessor32<float, 3, torch::RestrictPtrTraits>(),
			charges.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
			species.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
			element_types.packed_accessor32<int, 2, torch::RestrictPtrTraits>(),
...
//printf("Shared mem requested: %d bytes\n", shared_mem_size);
	egto_atomic_representation_cuda_ver4<<<currBatch, nthreads, shared_mem_size * sizeof(float)>>>(
			coordinates.packed_accessor32<float, 3, torch::RestrictPtrTraits>(),
			charges.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
			species.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
			element_types.packed_accessor32<int, 2, torch::RestrictPtrTraits>(),
			elemental_vectors.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
			blockAtomIDs.packed_accessor32<int, 1, torch::RestrictPtrTraits>(),
			blockMolIDs.packed_accessor32<int, 1, torch::RestrictPtrTraits>(),
			neighbourlist.packed_accessor32<int, 3, torch::RestrictPtrTraits>(),
			nneighbours.packed_accessor32<int,2, torch::RestrictPtrTraits>(),
...
//printf("CURR BATCH: %i, NTHREADS: %i\n", currBatch, nthreads);
	...
	egto_atomic_representation_derivative_cuda_ver4<<<currBatch, nthreads, shared_mem_size * sizeof(float)>>>(
			coordinates.packed_accessor32<float, 3, torch::RestrictPtrTraits>(),
			charges.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
			species.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
			element_types.packed_accessor32<int, 2, torch::RestrictPtrTraits>(),
