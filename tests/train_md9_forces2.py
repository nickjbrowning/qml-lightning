...
if __name__ == "__main__":
    ...
    for i, (coordinates, charges, energies, forces) in enumerate(zip(train_coordinates, train_charges, train_energies, train_forces)):
        ...
        for i in range(nbatch):
            ...
            GtrainY += torch.matmul(Gtrain_derivative.reshape(zbatch * natoms * 3, nfeatures).T, batch_forces.double().flatten()[:, None])
        
            # ZTZ += torch.matmul(Ztrain.T, Ztrain) + torch.matmul(Gtrain_derivative.reshape(zbatch * natoms * 3, nfeatures).T,
            #                                             Gtrain_derivative.reshape(zbatch * natoms * 3, nfeatures))
            
            ZTZ += torch.matmul(Gtrain_derivative.reshape(zbatch * natoms * 3, nfeatures).T,
                                                         Gtrain_derivative.reshape(zbatch * natoms * 3, nfeatures))
    ...
    for i, (coordinates, charges, energies, forces) in enumerate(zip(test_coordinates, test_charges, test_energies, test_forces)):
        ...
        gto_test, gto_test_grad = get_elemental_gto(coordinates, charges, torch.from_numpy(species).cuda().float(), ngaussians, eta, lmax, rcut, gradients=True)
        ...
        energy_mae = torch.mean(torch.abs(E - energies))
        
        avg_energy_mae += energy_mae.item()
        print("Energy MAE:", energy_mae)
        print("Force MAE:", torch.mean(torch.abs(F - forces.flatten())))
        
    print ("Average Energy MAE", avg_energy_mae / len(datas))
